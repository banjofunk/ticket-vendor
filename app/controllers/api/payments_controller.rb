...
class Api::PaymentsController < ApplicationController
  ...
  def send_payment
    ...
    unless result.success?
      ...
      raise PaymentError, "#{result.transaction_status} -- #{reason_text}"
    end

    render json: {bt_id: result.transaction.id}
  end
...
#           bama_count += 1
#         elsif [68, 66].include? promotion.id
#           adult_promotion = Promotion.find(promotion.id)
#           adult_price = adult_promotion.apply_taxes
#           adult_item_total = adult_price * promo["ticketData"][pirate_count]['adultTix'].to_i
#
#           free_kids = (adult_item_total/1).floor
#           child_promotion = Promotion.find(promotion.id)
#           child_price = child_promotion.apply_taxes
#           child_item_total = child_price * promo["ticketData"][pirate_count]['childTix'].to_i
#           adjusted_children = child_item_total - free_kids
#           adjusted_children = 0 if adjusted_children < 0
#           item_total = adjusted_children + adult_item_total
#           pirate_count += 1
...
#           promotions = attraction.try(:redemption_promotion) || []
#           if promotions.count === 1 && attraction.id != 5
...
#         end
#       end
#       transaction.admissions.where(promotion_id: [68, 66]).each_with_index do |pirate, idx|
#         cart_data.each do |item|
#           if [68, 66].include? item['id']
#             pirate.ticket_data = item["ticketData"][idx]
#             pirate.save
#           end
#         end
#       end
...
#         summary[:company] = promotion.attraction.name
...
#             tax_amount = (promotion.net_price.to_f/100) * (tax.amount.to_f/1000)
#           else
#             tax_amount = tax.amount.to_f/100
#           end
#           summary[:taxes][tax.id.to_s] = admission.redemption_code.try(:code) ? tax_amount : "$0.00"
#         end
